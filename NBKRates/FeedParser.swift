...
class FeedParser: NSObject, XMLParserDelegate {
    ...
    func parseEndOfRSS2Element(_ elementName: String, qualifiedName qName: String!) {
        ...
        else if self.currentPath == "/rss/channel/item/change"{
            self.currentFeedItem?.feedChange = self.currentElementContent
        }
        else if self.currentPath == "/rss/channel/item/quant"{
            self.currentFeedItem?.feedQuantity = self.currentElementContent
        }
    ...
    func retrieveDateFromDateString(_ dateString: String, feedType: FeedType?) -> Date {
        ...
        if currentDate == nil {currentDate = Date(fromString: dateString, format: .custom("dd.MM.yy"))}
